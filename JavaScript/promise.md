...
> <b>promise는 javascript에서 비동기를 간편하게 처리할 수 있도록 도와주는 객체이다.</b>
...
- <b>여기서 최종 결과를 꺼내는 타이밍은 정할 수 있다. 즉 나중에 쓸 수 있다! (then, catch를 통해)</b>

- 내가 결과를 받지 않으면 내용이 모두 실행(수행)되었음에도 결과를 아직 반환하지 않은 객체로 남게된다.

```javascript
// setTimeout을 promise로 래핑했다면
const promise = setTimeoutPromise(3000);

// 무언가 다른 일을 하다가~

promise.then(() => {
  // 이 때 resolve된 값으로 무언가 할 수도 있지
})

// * 참고) setTimeout의 콜백은 시간이 지나면 즉시 실행되지만,
// * Promise로 래핑하면 그 결과를 .then()으로 "나중에" 처리할 수 있다.
```

- <b>주의할 것은 promise를 만드는 순간, 우리가 전달한 콜백함수는 바로 실행이 된다.</b><br>

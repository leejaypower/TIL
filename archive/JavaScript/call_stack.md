# call stack
코드는 위에서부터 아래로 실행된다.<br>
이 때 a 함수를 호출하면 함수의 선언부가 실행되는데, 이 때 콜스택에 a라는 함수가 쌓이게 된다.<br>
만약 a 함수안에서 다른 함수를 호출한다면 a 함수 위로 (다음으로) 스택에 쌓인다.<br>
a 함수 안에서 b, c 등의 함수가 실행된다면 b, c 함수가 종료될 때 하나씩 스택에서 제거된다.<br> 
